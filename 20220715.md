...
如果我们Search Space 有重复子问题的话，可以记录下这些子问题的答案来保证不会重复计算多次。所以DP也被称为Search + Memoization。
...
对于单个Array或者String来说，一般只能2种状态定义：
...
比如我们知道子问题j的答案是ture -> we can break the word.substring(0, j)
...
function dfs(s, n, memo) {
    ...
    let res = 0, x = s.charAt(n - 1), y = s.charAt(n - 2);
    if (x !== '0') {
        res += dfs(s, n - 1, memo);
    }
    let yx = (y.charCodeAt() - '0'.charCodeAt()) * 10 + (x.charCodeAt() - '0'.charCodeAt());
    if (yx >= 10 && yx <= 26) {
        res += dfs(s, n - 2, memo);
    }
    memo[n] = res;
...
function numDecodings(s) {
    ...
    dp[0] = dp[1] = 1;
    for (let i = 2; i <= n; i++) {
        const curr = s.charAt(i - 1), prev = s.charAt(i - 2);
        const code = (prev.charCodeAt() - '0'.charCodeAt()) * 10 + (cur.charCodeAt() - '0'.charCodeAt());
        if (cur === '0') {
...
function uniquePathsWithObstacles(obstacleGrid) {
    ...
    const memo = Array.from({ length: m }, () => Array.from({ length: n }).fill(null));
...
function uniquePathsWithObstacles(obstacleGrid) {
    const m = obstacleGrid.length, n = obstacleGrid[0].length;
    const memo = Array.from({ length: m + 1 }, () => Array.from({ length: n + 1 }).fill(null));
    memo[1][1] = 1;
...
function longestCommonSubsequence(text1, text2) {
    const m = text1.length, n = text.length;
    const memo = Array.from({ length: m }, () => Array.from({ length: n }).fill(null));
...
function longestCommonSubsequence(text1, text2) {
    ...
    const memo = Array.from({ length: m + 1 }, () => Array.from({ length: n + 1 }).fill(0));
